...
class SortSourceService:
    ...
    async def sort_sources(self, query: str, search_results: List[dict]):
        try:
            ...
            # Run the CPU-intensive sorting logic in a separate thread
            return await asyncio.to_thread(self._sort_sources_sync, query, search_results)
        except Exception as e:
            print(e)
            return []

    def _sort_sources_sync(self, query: str, search_results: List[dict]):
        relevant_docs = []
